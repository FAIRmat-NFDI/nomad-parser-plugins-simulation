...
from tests.parsers.common import SimulationParserTestSuite, approx


class LobsterParserIntegrationSuite(SimulationParserTestSuite):
    """LOBSTER-specific configuration of the shared integration suite."""
...
class TestFeArchive(LobsterParserIntegrationSuite):
    ...
    def test_archive_contract(self, fe_archive):  # noqa: PLR0915
        ...
        assert model_system.positions.to('angstrom').magnitude[0][1] == approx(
            1.41588779
        )
        ...
        assert cohp.x_lobster_cohp_distances[0].to('angstrom').magnitude == approx(
            2.83178
        )
        assert cohp.x_lobster_cohp_distances[13].to('angstrom').magnitude == approx(
            2.45239
        )
        assert cohp.x_lobster_cohp_distances[19].to('angstrom').magnitude == approx(
            2.83178
        )
        ...
        assert coop.x_lobster_coop_distances[0].to('angstrom').magnitude == approx(
            2.83178
        )
        assert coop.x_lobster_coop_distances[13].to('angstrom').magnitude == approx(
            2.45239
        )
        assert coop.x_lobster_coop_distances[19].to('angstrom').magnitude == approx(
            2.83178
        )
        ...
        assert coop.x_lobster_average_integrated_coop_values[0][
            200
        ].magnitude == approx(-0.12265)
        assert coop.x_lobster_average_integrated_coop_values[1][
            200
        ].magnitude == approx(-0.10557)
        ...
        # TODO: assert dos_up.n_energies == 201
        ...
        # TODO: assert np.shape(dos_up.total[0].value_integrated) == (201,)
        # TODO: assert dos_up.total[0].value_integrated[10] == approx(0.0 + 18)
        # TODO: assert dos_up.total[0].value_integrated[188] == approx(11.07792 + 18)
        # TODO: assert dos_down.total[0].value_integrated[200] == approx(10.75031 + 18)

        # DOSCAR.lobster atom and lm-projected dos
        assert len(dos_up.projected_dos) == 12 and len(dos_down.projected_dos) == 12
        # TODO: assert (
        # TODO:     dos_up.atom_projected[0].atom_index == 0
        # TODO:     and dos_up.atom_projected[6].atom_index == 1
        # TODO: )
        # TODO: assert dos_up.atom_projected[0].m_kind == 'real_orbital'
        # TODO: assert (dos_up.atom_projected[4].lm == [2, 1]).all()
        assert np.shape(dos_up.projected_dos[11].value) == (201,)
        assert dos_up.projected_dos[5].value[190].to('1/eV').magnitude == approx(
            0.00909
        )
        assert dos_down.projected_dos[5].value[190].to('1/eV').magnitude == approx(
            0.29205
        )


class TestNaClArchive(LobsterParserIntegrationSuite):
    ...
    def test_archive_contract(self, nacl_archive):  # noqa: PLR0915
        ...
        assert cohp.x_lobster_cohp_distances[47].to('angstrom').magnitude == approx(
            2.82550
        )
        assert cohp.x_lobster_cohp_distances[71].to('angstrom').magnitude == approx(
            3.99586
        )
        ...
        assert coop.x_lobster_coop_distances[12].to('angstrom').magnitude == approx(
            2.82550
        )
        assert coop.x_lobster_coop_distances[71].to('angstrom').magnitude == approx(
            3.99586
        )
        ...
        assert mulliken.contributions[0].value.to(
            'elementary_charge'
        ).magnitude == approx(0.78)
        assert mulliken.contributions[7].value.to(
            'elementary_charge'
        ).magnitude == approx(-0.78)
        ...
        assert loewdin.contributions[0].value.to(
            'elementary_charge'
        ).magnitude == approx(0.67)
        assert loewdin.contributions[7].value.to(
            'elementary_charge'
        ).magnitude == approx(-0.67)
        ...
        # TODO: assert dos.n_energies == 201
        ...
        # TODO: assert np.shape(dos.value_integrated) == (201,)
        # TODO: assert dos.value_integrated[10] == approx(7.99998 + 80)
        # TODO: assert dos.value_integrated[160] == approx(27.09225 + 80)
        # TODO: assert dos.value_integrated[200] == approx(31.99992 + 80)

        # DOSCAR.lobster atom and lm-projected dos
        assert len(dos.projected_dos) == 20
        # TODO: assert dos.projected_dos[0].atom_index == 0
        # TODO: assert dos.projected_dos[19].atom_index == 7
        # TODO: assert dos.projected_dos[5].m_kind == 'real_orbital'
        # TODO: assert (dos.projected_dos[17].lm == [1, 2]).all()
...
class TestHfV2Archive(LobsterParserIntegrationSuite):
    archive_fixture = 'hfv2_archive'
    ...
    def test_archive_contract(self, hfv2_archive):  # noqa: PLR0915
        ...
        assert cohp.x_lobster_cohp_distances[55].to('angstrom').magnitude == approx(
            2.55809
        )
        ...
        assert coop.x_lobster_coop_distances[47].to('angstrom').magnitude == approx(
            2.60684
        )
        assert coop.x_lobster_coop_distances[55].to('angstrom').magnitude == approx(
            2.55809
        )
...
class TestNiArchive(LobsterParserIntegrationSuite):
    archive_fixture = 'ni_archive'
...
class TestSiArchive(LobsterParserIntegrationSuite):
    ...
    def test_archive_contract(self, si_archive):  # noqa: PLR0915
        ...
        # TODO: assert method.x_lobster_basis_functions == {
        # TODO:     'Si': [
        # TODO:         '3s',
        # TODO:         '3p_y',
        # TODO:         '3p_z',
        ...
        assert cohp.x_lobster_cohp_distances[23].to('angstrom').magnitude == approx(
            4.53443
        )
        ...
        for spin in [0, 1]:
            for ix, icohp in enumerate(
                cohp.x_lobster_integrated_cohp_at_fermi_level[spin]
            ):
                assert np.isclose(
                    icohp.magnitude,
