...
def test_get_data_files():
    ...
    assert os.path.basename(data_files[0]) == 'data.64xmethane_from_restart'
    assert os.path.isabs(data_files[0])

    # Test 2: Scan directory by pattern (no read_data command)
    with tempfile.TemporaryDirectory() as tmpdir:
        # Create test data files
        data_file = os.path.join(tmpdir, 'data.polymer')
        with open(data_file, 'w') as f:
            f.write('LAMMPS data file. CGCMM style. atom_style full generated by VMD\n')
            f.write('1134 atoms\n')
            f.write('1188 bonds\n')

        other_data = os.path.join(tmpdir, 'initial.dat')
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        # Create file without standard naming but with LAMMPS header
        data_file = os.path.join(tmpdir, 'structure.lmp')
        with open(data_file, 'w') as f:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        # Create multiple data-like files
        good_file = os.path.join(tmpdir, 'data.good')
        with open(good_file, 'w') as f:
            f.write('LAMMPS data file\n')
            f.write('100 atoms\n')

        bad_file = os.path.join(tmpdir, 'data.bad')
