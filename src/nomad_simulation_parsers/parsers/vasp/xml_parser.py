...
class VasprunParser(XMLParser):
    ...
    def get_eigenvalues(self, source: dict[str, Any] | None) -> list[dict[str, Any]]:
        ...
        for spin_channel, kpoint_sets in enumerate(spin_sets):
            ...
            occs = data[:, :, 1]
            entry = dict(
                value=eigs,  # Changed from 'eigenvalues' to 'value'
                occupation=occs,  # Changed from 'occupations' to 'occupation'
                n_levels=eigs.shape[1],  # Changed from 'n_bands' to 'n_levels'
