...
class GromacsMDAnalysisParser(MappingParser):
    ...
    def get_subsystems_from_dict(
        ...
        if 'step' in source and 'sub_systems' not in source:
            first_step = (
                self._trajectory_steps_sampled[0]
                if self._trajectory_steps_sampled
                else None
            )
            if source.get('step') == first_step:
    ...
    def get_particle_states(
        ...
        return self._particle_parameters if step == first_step else []
    ...
    def get_configurations(self) -> list[dict[str, Any]]:
        ...
        configurations = []
        for n, step in enumerate(self._trajectory_steps_sampled):
            config = dict(
                step=step,
            ...
            if n == 0:
                ...
                lv = self.data_object.get_lattice_vectors(n)
                if lv is not None:
