...
class OutParser(TextParser):
    ...
    def init_quantities(self):
        ...
        def to_parameters(val_in):
            ...
            n_val = 2
            return {
                line[0].strip(): line[1].strip() for line in lines if len(line) == n_val
            }
        ...
        method_quantities = [
            ...
            Quantity(
                ...
                r'R A N G E S    F O R    A T O M I C    O R B I T A L S'
                r'\s+\=+([\s\S]+?)\={10}',
                sub_parser=TextParser(
                    quantities=[
                        Quantity(
            ...
            Quantity(
                'k_space_sampling',
                r'(?:K \- S P A C E   S A M P L I N G|K\-space integration)'
                r'([\s\S]+?)\={5}',
                sub_parser=TextParser(
                    quantities=[
                        Quantity(
                ...
                sub_parser=TextParser(
                    quantities=[
                        Quantity(
                            ...
                            r'DIIS settings info for DIRAC'
                            r' +\*\s+((?:[\w ]+?  \w\S*\s+)+)',
                            str_operation=to_parameters,
                        ),
                        Quantity(
                            'x_ams_diis_settings_scf',
                            r'DIIS settings info for SCF'
                            r' +\*\s+((?:[\w ]+?  \w\S*\s+)+)',
                            str_operation=to_parameters,
                        ),
                        Quantity(
        ...
        calculation_quantities = (
            ...
            + [
                ...
                Quantity(
                    'forces',
                    rf'(E N E R G Y +G R A D I E N T S\s+\=+[\s\S]+?FINAL'
                    rf'[\s\S]+?){RE_N} *{RE_N}',
                    sub_parser=TextParser(
                        quantities=[
                            Quantity(
                                'p_matrix',
                                rf'P Matrix\s+((?:\d+ +[A-Z][a-z]*'
                                rf' +{RE_FLOAT} +{RE_FLOAT} +{RE_FLOAT}\s+)+)',
                                str_operation=str_to_forces,
                            ),
                            Quantity(
                                'electronic_kinetic',
                                rf'Kinetic energy\s+((?:\d+ +[A-Z][a-z]*'
                                rf' +{RE_FLOAT} +{RE_FLOAT} +{RE_FLOAT}\s+)+)',
                                str_operation=str_to_forces,
                            ),
                            Quantity(
                                'xc',
                                rf'XC energy\s+((?:\d+ +[A-Z][a-z]*'
                                rf' +{RE_FLOAT} +{RE_FLOAT} +{RE_FLOAT}\s+)+)',
                                str_operation=str_to_forces,
                            ),
                            Quantity(
                                'electrostatic',
                                rf'Electrostatic energy\s+((?:\d+ +[A-Z][a-z]*'
                                rf' +{RE_FLOAT} +{RE_FLOAT} +{RE_FLOAT}\s+)+)',
                                str_operation=str_to_forces,
                            ),
                            Quantity(
                                'pair_interactions',
                                rf'Pair interactions\s+((?:\d+ +[A-Z][a-z]*'
                                rf' +{RE_FLOAT} +{RE_FLOAT} +{RE_FLOAT}\s+)+)',
                                str_operation=str_to_forces,
                            ),
                            Quantity(
                                'dispersion',
                                rf'Dispersion\s+((?:\d+ +[A-Z][a-z]*'
                                rf' +{RE_FLOAT} +{RE_FLOAT} +{RE_FLOAT}\s+)+)',
                                str_operation=str_to_forces,
                            ),
                            Quantity(
                                'total',
                                rf'FINAL GRADIENTS\s+((?:\d+ +[A-Z][a-z]*'
                                rf' +{RE_FLOAT} +{RE_FLOAT} +{RE_FLOAT}\s+)+)',
                                str_operation=str_to_forces,
                            ),
                        ]
                ...
                Quantity(
                    ...
                    sub_parser=TextParser(
                        quantities=[
                            Quantity(
                                'total',
                                rf'Total Gradients \(hartree/bohr\)\s+'
                            ...
                            Quantity(
                                'electronic',
                                rf'Electronic Gradients \(hartree/bohr\)\s+'
                                rf'Index.+\s+((?:\d+ +[A-Z][a-z]*'
                                rf' +{RE_FLOAT} +{RE_FLOAT} +{RE_FLOAT}\s+)+)',
                                str_operation=str_to_forces,
                            ),
                            Quantity(
                                'electrostatic',
                                rf'Coulomb Gradients \(hartree/bohr\)\s+'
                                rf'Index.+\s+((?:\d+ +[A-Z][a-z]*'
                                rf' +{RE_FLOAT} +{RE_FLOAT} +{RE_FLOAT}\s+)+)',
                                str_operation=str_to_forces,
                            ),
                            Quantity(
                                'nuclear_repulsion',
                                rf'Repulsion Gradients \(hartree/bohr\)\s+'
                                rf'Index.+\s+((?:\d+ +[A-Z][a-z]*'
                                rf' +{RE_FLOAT} +{RE_FLOAT} +{RE_FLOAT}\s+)+)',
                                str_operation=str_to_forces,
                            ),
                            Quantity(
                                'dispersion',
                                rf'Dispersion Gradients \(hartree/bohr\)\s+'
                                rf'Index.+\s+((?:\d+ +[A-Z][a-z]*'
                                rf' +{RE_FLOAT} +{RE_FLOAT} +{RE_FLOAT}\s+)+)',
                                str_operation=str_to_forces,
                            ),
                ...
                Quantity(
                    'self_consistency',
                    r'S C F   P R O C E D U R E\s*\*\s*\*+\s*'
                    r'([\s\S]+?Self consistent error.+)',
                    sub_parser=TextParser(
                        quantities=[
                            Quantity(
                                'energy_change',
                ...
                Quantity(
                    'mulliken_populations',
                    r'(Mulliken Charges\s+\-+[\s\S]+?Total.+)',
                    sub_parser=TextParser(
                        quantities=[
                            Quantity(
                                'atom',
                                rf'Index +Atom +Charge.+\s+((?:\d+ +\w+'
                                rf' +{RE_FLOAT} +{RE_FLOAT}\s+)+)',
        ...
        self._quantities = [
            ...
            Quantity(
                ...
                r'GEOMETRY OPTIMIZATION\s*\*([\s\S]+?)'
                r'(:?Timing|Performing a single pooint|\Z)',
                sub_parser=TextParser(
                    ...
                    + [
                        Quantity(
                            'step',
                            r'Geometry Convergence after Step'
                            r'\s*([\s\S]+?(?:dE\(predicted\)\:.+|\Z))',
                            repeats=True,
                            sub_parser=TextParser(quantities=calculation_quantities),
                        ),
                        Quantity(
...
class RKFParser(FileParser):
    def __init__(self, mainfile=None, logger=None, open=None):
        ...
        self._energies_map = {
            ...
            'x_ams_orbital_interaction_ts_correction_lda',
    ...
    def _parse_md(self, calc_results) -> None:
        if (mdhistory := self.data.get('MDHistory')) is None:
            return

        self._calc_type = 'molecular_dynamics'
            ...
            for n in range(n_entries):
                ...
                if (
                    ...
                    energies['total_potential'] = (
                        potential if isinstance(potential, float) else potential[ndata]
                    ) * ureg.hartree
                ...
                if (velocities := mdhistory.get(f'Velocities({n + 1})')) is not None:
                    calc_results['step'][n]['velocities'] = (
                        np.reshape(velocities, (len(self._labels), 3))
                        * ureg.bohr
                        / ureg.fs
                    )

    def _parse_geometry_optimization(self, calc_results) -> None:
        history = self.data.get('History', {})
        if history:
            self._calc_type = 'geometry_optimization'
        ...
        if (geoopt := self.data.get('GeoOpt')) is not None:
            if (gradients := geoopt.get('Gradients_CART')) is not None:
                self._results['forces_total'] = (
                    np.reshape(gradients, (len(self._labels), 3))
                    * ureg.hartree
                    / ureg.bohr
                )
            if (hessian := geoopt.get('Hessian_CART')) is not None:
                hessian = np.reshape(
                    hessian, (len(self._labels), 3, len(self._labels), 3)
                )
    ...
    def _parse_history(self, calc_results) -> None:
        ...
        for n, name in enumerate(files):
            rkf_files = search_files(name, os.path.dirname(self.mainfile))
            if not rkf_files:
    ...
    def parse_results(self) -> None:
        ...
        self._parse_geometry_optimization(calc_results)

        self._parse_md(calc_results)

        self._parse_history(calc_results)

        if self._calc_type is not None:
            self._results[self._calc_type] = calc_results
    ...
    def parse_properties(self) -> None:
        ...
        mulliken = properties.get('AtomCharge Mulliken')
        ...
        fermi = properties.get('FermiLevel')
        # if (fermi := properties.get('FermiLevel')) is not None:
        self._results['fermi_energy'] = fermi * ureg.hartree if fermi else fermi

        if (dipole := properties.get('Dipole')) is not None:
            dipole = np.sqrt(np.sum(np.array(dipole) ** 2))
            self._results['dipole_moment'] = (
                ([dipole] * ureg.elementary_charge * ureg.bohr).to('C * m').magnitude
            )

        self._results['band_gap'] = {}
        homo = properties.get('HOMO')
        self._results['band_gap']['energy_highest_occupied'] = (
            (homo * ureg.hartree) if homo else None
        )
        lumo = properties.get('LUMO')
        self._results['band_gap']['energy_lowest_unoccupied'] = (
            (lumo * ureg.hartree) if lumo else None
        )
        ...
        band_structure = self.data.get('BandStructure', {})
        ...
        band_gap = {}
        value = band_structure.get('BandGap')
        band_gap['value'] = value * ureg.hartree if value is not None else None
        top_valence_band = band_structure.get('TopValenceBand')
        band_gap['energy_highest_occupied'] = (
            top_valence_band * ureg.hartree if top_valence_band is not None else None
        )
