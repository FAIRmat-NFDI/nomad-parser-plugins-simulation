...
add_mapping_annotations(general.Simulation.m_def, INPUT_XML_KEY, '@')
add_mapping_annotations(general.Simulation.m_def, EIGVAL_KEY, '@')
add_mapping_annotations(general.Simulation.m_def, BANDSTRUCTURE_XML_KEY, '@')
add_mapping_annotations(general.Simulation.m_def, DOS_XML_KEY, '@')


class Simulation(general.Simulation):
    add_mapping_annotations(general.Simulation.program, INFO_KEY, '.@')
    # DFT method
    add_mapping_annotations(
        model_method.DFT.m_def, INFO_KEY, '.initialization.xc_functional'
    )
    add_mapping_annotations(model_method.DFT.m_def, INPUT_XML_KEY, '.input.groundstate')
    add_mapping_annotations(model_method.DFT.m_def, BANDSTRUCTURE_XML_KEY, '.@')
    add_mapping_annotations(
    ...
    add_mapping_annotations(general.Simulation.outputs, EIGVAL_KEY, '.@')
    add_mapping_annotations(general.Simulation.outputs, BANDSTRUCTURE_XML_KEY, '.@')
    add_mapping_annotations(general.Simulation.outputs, DOS_XML_KEY, '.@')


class Program(general.Program):
    add_mapping_annotations(general.Program.version, INFO_KEY, '.program_version')
...
class XCFunctional(model_method.XCFunctional):
    add_mapping_annotations(model_method.XCFunctional.libxc_name, INFO_KEY, '.libxc')
    add_mapping_annotations(
...
class ModelSystem(model_system.ModelSystem):
    add_mapping_annotations(model_system.AtomicCell.m_def, INFO_KEY, '.@')
    add_mapping_annotations(model_system.ModelSystem.positions, INFO_KEY, '.positions')
    add_mapping_annotations(model_system.AtomsState.m_def, INFO_KEY, '.atoms')
...
class AtomsState(atoms_state.AtomsState):
    add_mapping_annotations(atoms_state.AtomsState.chemical_symbol, INFO_KEY, '.symbol')


class Outputs(outputs.Outputs):
    add_mapping_annotations(outputs.Outputs.total_energies, INFO_KEY, '.@')
    add_mapping_annotations(
...
class TotalForce(properties.forces.TotalForce):
    add_mapping_annotations(properties.forces.TotalForce.value, INFO_KEY, '.forces')
