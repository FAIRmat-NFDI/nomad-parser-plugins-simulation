...
from nomad_simulations.schema_packages import general, model_system, model_method, outputs, numerical_settings
...
class ModelSystem(model_system.ModelSystem):
    add_mapping_annotation(model_system.ModelSystem.positions, NETCDF_KEY, ('get_positions', []), unit='angstrom')
    add_mapping_annotation(model_system.ModelSystem.lattice_vectors, NETCDF_KEY, ('get_lattice_vectors', []))
    add_mapping_annotation(model_system.AtomsState.m_def, NETCDF_KEY, ('get_labels', []))

class KMesh(numerical_settings.KMesh):
    add_mapping_annotation(numerical_settings.KMesh.all_points, NETCDF_KEY, '.@')

class KSpace(numerical_settings.KSpace):
    add_mapping_annotation(numerical_settings.KSpace.k_mesh, NETCDF_KEY, ('get_kpoints', []))
...
class Outputs(outputs.Outputs):
    add_mapping_annotation(outputs.Outputs.electronic_eigenvalues, NETCDF_KEY, ('get_eigenvalues', []), cache=True)
    add_mapping_annotation(outputs.Outputs.electronic_eigenvalues, OUT_KEY, '.eigenvalues')

class Simulation(general.Simulation):
    add_mapping_annotation(general.Simulation.wall_start, OUT_KEY, ('get_wallstart', ['.date_start']))
    ...
    add_mapping_annotation(general.Simulation.outputs, OUT_KEY, ('get_outputs', ['.core_variables_setup.energies_occupations', '.modules', '.transferred_momenta']))
